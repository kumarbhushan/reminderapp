...
function createStuffTable () {
  ...
  try {
    db.transaction(function (tx) {
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS MyStuff(UId INTEGER NOT NULL PRIMARY KEY, ImageURI TEXT UNIQUE)',
        [], nullHandler, errorHandler)
    }, errorHandler, function (data) {
      db.transaction(function (_tx) {
        _tx.executeSql(
          'INSERT INTO MyStuff(ImageURI) VALUES (?),(?),(?),(?),(?),(?),(?),(?),(?),(?),(?),(?),(?),(?),(?),(?)',
          ['img/gallery/1.jpg',
            'img/gallery/2.jpg',
            'img/gallery/3.jpg',
            'img/gallery/4.jpg',
